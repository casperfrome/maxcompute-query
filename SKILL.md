...
## 连接说明
凭证与端点集中在 `scripts/config.py`（两个脚本共用的单一来源；首次使用时由 `scripts/config.example.py` 复制而来）。可用环境变量 `ODPS_ACCESS_ID/ODPS_SECRET/ODPS_PROJECT/ODPS_ENDPOINT` 覆盖，无需改代码。
