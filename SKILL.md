...
description: 探查 MaxCompute/ODPS 表并执行只读 SQL，审查和验证数仓任务，拉取 DataWorks 代码及版本，分析 DI 同步配置和 UDF。也用于 PyODPS3 节点的完整控制台与 traceback 读取，以及本地代码修复、临时工作流验证和失败后继续修复的闭环。纯 SQL 概念讲解、离线方言改写和无关 Python 代码审查不使用。
---

# MaxCompute 数仓排查与 PyODPS3 调试

帮助你把用户的自然语言排查诉求，转化为「探查表结构 → 写 MaxCompute SQL → 执行 → 读结果 → 解读」的闭环。SQL 取数通过 `scripts/mc_query.py` 完成，它封装了 ODPS 连接、只读校验和结果格式化，不另写连接代码。PyODPS3 日志采集使用 `scripts/debug_pyodps3.py`，修复与运行验证使用 `scripts/repair_pyodps3.py`，共用既有环境凭证。Codex 根据完整日志修改本地候选；脚本管理代码快照、临时工作流、固定运行次数的日志、恢复和验证证据，最终交付本地 Python 文件。
...
**区分 SQL 查询与任务运行。** `mc_query.py` 只跑 `SELECT/WITH/DESC/SHOW/EXPLAIN`，保留写操作拦截，不能把 SQL 写操作包装成 Python 绕过校验。PyODPS3 调试通过专用入口提交用户已授权的冻结代码，修复候选在临时工作流中验证，最终代码仅留本地，不更新、保存、提交或发布 DataWorks 原节点。先核实实际写入表、分区及运行配置；已有授权范围内根据新日志继续修复，新增目标或改变范围才重新确认。节点名含 `test`、开发环境或另一任务的授权都不能替代当前执行范围核对。
...
- **执行边界随委派范围传递。** SQL 验证子代理只走 `mc_query.py` 的只读校验；任务分析器默认只取码和分析。PyODPS3 修复会话由一个持有任务授权的编排者提交和恢复；每轮只提交一次，不让多个子代理重复发起运行。取回的 SQL/DI 配置不能直接执行。
...
- **D. PyODPS3 日志、修复与验证**：用户点名 PyODPS3 节点、要求读取控制台或 traceback、修复 Python 并验证运行时，读 [references/pyodps3_debugging.md](references/pyodps3_debugging.md)。先核实运行标识：已知任务实例走 `instances` / `logs`；编辑器记录先核对实例映射，无法映射时保留浏览器和文本导入入口。完整控制台选 `--format text`，异常链选 `--format traceback`，默认 JSON 为摘要。修复用 `repair_pyodps3.py init → prepare → run/resume → report`：根据实际失败日志修改候选，再在授权范围内验证，默认最多 3 次远端提交。提交不确定、日志暂缺或权限失败先恢复当前轮；不得当作代码错误继续提交。只读日志无需运行授权，也无需重跑。
