...
description: 通过辅助脚本探查表结构、编写并执行 MaxCompute (ODPS) 只读 SQL 来排查数仓/数据问题，并用真实数据为结论佐证。三类场景都要触发：(1) 任何需要查数据仓库才能回答的情况——排查数仓/数据异常、核对某个指标或数量、想知道某张表/某个字段的数据情况、对某现象取数验证、统计分布/占比/Top N、确认有没有脏数据或空值等；(2) 审查/排查/review/debug 一段已有的 MaxCompute/ODPS/数仓 SQL（任务、脚本、作业、.sql 或 .txt 文件）——找逻辑漏洞、找 bug、检查口径或数据正确性、解释结果为什么不对；(3) 修改/改造/重构/调口径一段已有的在线数仓 SQL 任务——改唯一键/粒度、加去重、改 JOIN、增删字段、调过滤条件等，因为修改前要先用真实数据核实前提（粒度是不是真的会膨胀、某字段是不是真有重复、依赖的排序/格式假设是否成立），修改后也要用数据验证结果（新唯一键是否真唯一、行数增减是否符合预期）。这些可疑点（建表/依赖顺序导致读到旧数据、窗口 PARTITION BY 粒度错误、JOIN 缺键导致膨胀、缺分区过滤、NULL/重复/口径偏差）大多能用线上真实数据证实或证伪。即使用户只说"排查/看看这个 SQL 任务有没有问题/逻辑漏洞""帮我把这个任务的粒度改成 X/帮我改下这个 SQL"而没明说"写 SQL""查数据""验证一下"，也要触发并主动取数佐证前提与结果，而不是只做静态阅读或闭眼改写。即使用户只给出表名/任务名而没有贴出 SQL 文本（如"帮我看看 dws_com_sys_oa_archived_time_risk_wide_df 这个任务有没有逻辑问题"），也要触发，并先用辅助脚本从 DataWorks 自动拉取该任务的线上 SQL 代码，再进入审查/修改，而不是反过来让用户把 SQL 贴出来。拉取到的任务既可能是 ODPS SQL，也可能是**数据集成（离线同步 / 同步任务，DataWorks 数据开发里的 DI 节点，常见命名如 to_holo_..._di）**——后者 `fetch_task_sql.py` 会自动识别并解读出「源表→目标表、写入模式、reader/writer 列按位置映射对照」的精简摘要；用户说"看看这个数据集成任务把哪张表同步到哪""这个离线同步/同步任务源跟目标对得上吗""这个 to_holo_..._di 任务列有没有错位""这张 Holo 表是从哪同步过来的"时也要触发，并主动拉取、解读该同步配置。用户提到任务的"历史版本/上一版/之前那版代码/某次提交/版本对比/回退到某版/最近改了什么"时，也要触发，并用 `fetch_task_sql.py` 的版本子命令（`--list-versions` 列版本、`--get-version N` 取某版完整代码、`--diff A B` 对比两版差异）拉取历史版本代码。涉及 ODPS、MaxCompute、ds 分区表、MAX_PT、dwd/dws/ads 等数仓表名/特征时尤其适用。边界（不触发）：纯 SQL 语法或概念讲解、与线上数仓无关的纯方言互转（如业务库 MySQL↔PostgreSQL 改写、没有可连的线上表）、非 SQL 的代码 review。
---
...
当用户只给了表名/任务名、没有贴出 SQL（如「看看 `dws_xxx` 这个任务有没有问题」「帮我改下 `xxx` 这个任务」）时，先用 `scripts/fetch_task_sql.py <表名>` 主动把该任务的线上 SQL 代码拉下来，再进入审查/修改——别停在那里反过来问用户要 SQL。详见下文「拉取线上任务 SQL」。
...
- **能查就别问。** 表名/字段名/分区字段名拿不准、SQL 报错、「是不是真有重复 / 某字段是不是真恒为 NULL」——这些数据里有答案，去 `desc`/`sample`/`list-tables`/跑一条 SQL 就知道了。这种地方**自己查清，绝不打扰用户**。能查的去查，才是这个 skill 的灵魂；遇事就问会让它变得没用。
- **数据判不了、取决于用户的，就问。** 有些分叉无论怎么查数据都定不了，因为它取决于用户想要什么、用户怎么定义——这时**硬猜的代价很大**：整轮排查/改造会架在一个错误前提上，给出的结论或新 SQL 看着完整却是错的，比停下来问一句糟得多。这种地方用 **`AskUserQuestion`（提问框）** 发起结构化提问。
...
**从证据出发地问，别裸问。** 最好的做法不是一遇到模糊就甩个问题给用户，而是**先查到能框定选项的数据，再把选项摆进提问框**——这样既满足「用真实数据佐证」，用户也只需点选而不必从头解释。例：「`oa_archived` 命中这 3 张表：A 最新分区 1.2 亿行、B 30 万行、C 已半年没更新，你要查的是哪张？」。提问框给**具体可选项**，而不是一段「你是指 X 还是 Y？」的纯文字。
...
- **任务代码分析器** [agents/task-analyzer.md](agents/task-analyzer.md)：接任务/表名（或已贴出的 SQL），必要时用 `fetch_task_sql.py` 拉线上代码，通读后回传**结构化疑点清单**——而且每条「需数据佐证的疑点」都写成可直接交给验证器的「验证问题」。它让几百行任务 SQL 留在子代理里，不淹没你的上下文。

### 什么时候 fan-out、什么时候自己干

并行是为了提速和省上下文，不是为了热闹——给单个小查询套子代理只会徒增开销。判断标准：
...
- **基于证据下结论，不要转述。** 子代理必须回传真实 SQL + 真实数字；你要拿这些证据自己推理、自己合并，而不是把「子代理说没问题」直接抛给用户。skill 的灵魂是用真实数据佐证，别因为分工就丢了。
- **只读不变量照旧。** 子代理执行只走 `mc_query.py`（只读校验）；`fetch_task_sql.py` 拉回的任务代码（含 `DROP/CREATE/INSERT`）只供阅读分析，永不执行。
- **子代理问不到用户，疑问由你替它问。** 子代理没有提问框这个渠道。所以子代理一旦撞上「数据判不了、取决于用户」的决策点（口径定义、改造取舍、多个都合理的候选），约定是**不硬猜**，而是把它框定成「带选项的待澄清项」回传（两个模板的交付结构里都留了这个字段）。你收到后，**别把它的某个猜测当结论抛给用户——在给最终结论前，用 `AskUserQuestion`（提问框）替它把这个决策点问清楚**，拿到答复再合并。这一步是「不确定就问」能稳定发生的关键：因为不确定多半在子代理里冒出来，而只有你能弹提问框。

## 三种入口

本 skill 服务三类诉求，进来先判断属于哪一类：

- **A. 取数排查**：用户要的答案在数据里（多少行、什么分布、有没有空值、某指标对不对）。走下面的「工作流」。
- **B. 审查既有 SQL 任务**：用户给了一段 MaxCompute/ODPS SQL（脚本/作业/`.sql`/`.txt`），或**只点名了某张表/某个任务**（如「看看 `dws_xxx` 有没有逻辑问题」），要你找逻辑漏洞、bug、口径错误，或解释结果为什么不对。只给了名字没贴 SQL 时，**第一步先用 `fetch_task_sql.py` 把代码拉下来**。走「审查既有 SQL 任务」一节——核心是**别停在静态阅读，把能用数据验证的疑点真的查一遍**。
...
**⚠ 拉回来的 SQL 是「审查/修改对象」，不是拿去跑的查询。** 任务代码里通常含 `DROP/CREATE/INSERT` 等写语句——它只供你阅读分析，**绝不要丢给 `mc_query.py` 执行**（会被只读校验拦下，本就不该这么用）。要验证其中的前提或结果时，**另写只读 `SELECT`** 走 `mc_query.py`。
...
典型用法：先 `--list-versions` 看清有哪些版本、哪版是当前生产版（标 ★），再用 `--get-version N` 取目标版代码，或 `--diff A B` 定位两版之间的改动。版本号不存在会以退出码 4 友好报错——回到 `--list-versions` 核对可用版本号。**历史版本代码同样含 `DROP/CREATE/INSERT`，只供阅读分析，绝不要丢给 `mc_query.py` 执行。**
...
查找提示：DI 任务请用**任务名**（`to_holo_..._di`）查，别用 Holo 目标表名反查——按产出表反查只认 MaxCompute 表，查不到 Holo 目标。**拉回的同步配置同样只供阅读分析，不会、也不该丢给 `mc_query.py` 执行。**
...
2. **并行取数证实/证伪（验证器）。** 把上一步「需数据佐证的疑点」**在同一条消息里并行**交给多个 [verifier](agents/verifier.md) 子代理，每个查一条（务必带分区过滤）。常见疑点对应的查法：
   ...
   **默认主动取数验证，不要先问用户。** 这里说的「不要先问」专指**能用数据证实/证伪的疑点**——那是你该自己查的，别甩给用户。但如果验证过程中冒出**数据判不了、取决于用户口径/意图**的分叉（该按哪个字段算、口径含不含某种情况），仍按「什么时候停下来用提问框问用户」用提问框带选项问。疑点只有一两条时，自己跑 `mc_query.py` 也行，不必为并行而并行。
3. **合并结论。** 用验证器回传的**真实 SQL + 真实数字**，把「代码层结论」和「数据层验证」一起讲清楚：哪些是已证实的 bug、影响多少行/多少占比、哪些经查证其实没问题、修复建议是什么——别只转述子代理的判断。
...
1. **拉取 + 分析（任务代码分析器）。** 起 [task-analyzer](agents/task-analyzer.md)（场景填 `修改(C)` 并附「要改成什么」），让它回传两份清单：**改动前需验证的前提** 和 **改动后需自检的断言**——都已写成可直接交给验证器的形式。
   - **改造意图本身有歧义就先用提问框问清，再 dispatch 验证器。** 修改类诉求最容易藏着「取决于用户」的取舍：唯一键边界（含不含 `node_oper`）、按哪个时间字段取最早、并列/NULL 如何处理。这些数据判不了，硬挑一个会让后面整轮验证都架在错误的改法上、白跑一轮。若 task-analyzer 回传了「需用户澄清的决策点」，或你自己一眼看出这种分叉，先用 `AskUserQuestion`（提问框）带选项（连同已查到的字段格式/重复规模）问清，拿到答复再往下。
...
### 1. 厘清排查目标
先想清楚：用户到底要验证什么现象、什么指标、什么口径？这里要分清两种模糊（见「什么时候停下来用提问框问用户」）：**能查的别问**——哪张表、字段叫什么、最新分区是哪天，自己探查就有答案；**取决于用户的才问**——口径定义有歧义（如「城市」指注册地还是经营地）、范围有多种合理解读且默认值会改变结论时，用提问框带选项确认，别凭感觉挑一个就开查。能合理默认的（如默认取最新分区）就直接默认，不必为此发问。
...
一般 2-3 次内能修好。这里同样是两档制：**技术性报错**（表名/列名/语法/空结果）属于「能查就别问」，自己读懂、改 SQL、重试，别一报错就甩锅给用户——只有**连续几次仍卡在同一类错误**确实自助不了时，才把当前进展和具体报错上抛兜底。而问题涉及**业务口径你无法判断**（分不清该用哪个字段、不确定用户要的"城市"指注册地还是经营地）属于「取决于用户、该问」那一档——这不是等到卡死才问，而是一旦识别出这种分叉就用 `AskUserQuestion`（提问框）带选项问清。能自己查清的就查清，真正取决于用户的才上抛。
