...
拉回的内容可能是 ODPS SQL，也可能是**数据集成·离线同步任务**的 DataX 同步配置(JSON)。
后者会被自动识别并解读成精简摘要——只保留同步任务真正关心的四样：源表(+分区)、目标库.表、
写入模式、reader/writer 列按位置映射对照（含错位审查）；其余运行设置与原始 JSON 不再输出。
这类任务请用**任务名**（如 to_holo_..._di）查询，别用 Holo 目标表名反查。
...
def cmd_fetch(args):
    try:
        ...
        print(
            ...
            "若是数据集成(离线同步)任务，请用**任务名**（如 to_holo_..._di）匹配，"
            "不要用 Holo 目标表名反查——按产出表反查只认 MaxCompute 表。",
            file=sys.stderr,
        )
        sys.exit(4)
